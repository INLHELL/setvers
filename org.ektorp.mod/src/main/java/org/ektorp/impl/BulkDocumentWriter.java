...
public class BulkDocumentWriter {
	...
	public void write(Collection<?> objects, boolean allOrNothing, OutputStream out) {
		try {
			JsonGenerator jg = objectMapper.getFactory().createJsonGenerator(out, JsonEncoding.UTF8);
			jg.writeStartObject();
			if (allOrNothing) {
	...
	public InputStream createInputStreamWrapper(boolean allOrNothing, InputStream in) {
        ...
        try {
            ...
            JsonGenerator jg = objectMapper.getFactory().createJsonGenerator(byteArrayOutputStream, JsonEncoding.UTF8);
            jg.writeStartObject();
            if (allOrNothing) {
                jg.writeBooleanField("all_or_nothing", true);
            }
