...
public class FinalizableReferenceQueue {
	...
	static class DecoupledLoader implements FinalizerLoader {
		...
		private static final String LOADING_ERROR =
			"Could not load Finalizer in" + " its own class loader. Loading Finalizer in the current class loader"
		...
		public Class<?> loadFinalizer() {
			try {
				...
				FinalizableReferenceQueue.logger.log(Level.WARNING, DecoupledLoader.LOADING_ERROR, e);
				return null;
			}
		}
		...
		URL getBaseUrl() throws IOException, URISyntaxException {
			...
			final String finalizerPath = FinalizableReferenceQueue.FINALIZER_CLASS_NAME.replace('.', '/') + ".class";
	...
	private static final String FINALIZER_CLASS_NAME = "org.azeckoski.reflectutils.refmap.Finalizer";
	...
	static {
		...
		// If the LifeCycleManager has been activated, then load the Finalizer in
		// the current ClassLoader
		// so that the Finalizer thread can register to receive shutdown
		// notification
		if (LifecycleManager.isActive()) {
			finalizer = FinalizableReferenceQueue.loadFinalizer(new DirectLoader());
		}
		else {
			finalizer = FinalizableReferenceQueue.loadFinalizer(new SystemLoader(), new DecoupledLoader(), new DirectLoader());
		}
	...
	public FinalizableReferenceQueue() {
		...
		try {
			this.queue =
				(ReferenceQueue<Object>) FinalizableReferenceQueue.startFinalizer.invoke(null, FinalizableReference.class, this,
