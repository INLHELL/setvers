 ...
 * </ul>
 * 
 * @version $Revision: 2 $ $Date: 2008-10-01 11:04:26 +0100 (Wed, 01 Oct 2008) $
 * @since 1.8.0
 */
public class DefaultResolver implements Resolver {
	...
	public int getIndex(final String expression) {
		...
		for (int i = 0; i < expression.length(); i++) {
			...
			if ((c == DefaultResolver.NESTED) || (c == DefaultResolver.MAPPED_START)) {
				return -1;
			}
			else if (c == DefaultResolver.INDEXED_START) {
				final int end = expression.indexOf(DefaultResolver.INDEXED_END, i);
				if (end < 0) {
	 ...
	 * @throws IllegalArgumentException If the mapped property is illegally
	 *           formed.
	 */
	@Override
	public String getKey(final String expression) {
		...
		for (int i = 0; i < expression.length(); i++) {
			final char c = expression.charAt(i);
			if ((c == DefaultResolver.NESTED) || (c == DefaultResolver.INDEXED_START)) {
				return null;
			}
			else if (c == DefaultResolver.MAPPED_START) {
				final int end = expression.indexOf(DefaultResolver.MAPPED_END, i);
				if (end < 0) {
					throw new IllegalArgumentException("Missing End Delimiter");
				}
	...
	public String getProperty(final String expression) {
		...
		for (int i = 0; i < expression.length(); i++) {
			final char c = expression.charAt(i);
			if (c == DefaultResolver.NESTED) {
				return expression.substring(0, i);
			}
			else if ((c == DefaultResolver.MAPPED_START) || (c == DefaultResolver.INDEXED_START)) {
				return expression.substring(0, i);
			}
		}
		return expression;
	...
	public String next(final String expression) {
		...
		for (int i = 0; i < expression.length(); i++) {
			final char c = expression.charAt(i);
			if (indexed) {
				if (c == DefaultResolver.INDEXED_END) {
					return expression.substring(0, i + 1);
				}
			}
			else if (mapped) {
				if (c == DefaultResolver.MAPPED_END) {
					return expression.substring(0, i + 1);
				}
			}
			else {
	...
	public String remove(final String expression) {
		...
		if (expression.charAt(start) == DefaultResolver.NESTED) {
			start++;
		}
