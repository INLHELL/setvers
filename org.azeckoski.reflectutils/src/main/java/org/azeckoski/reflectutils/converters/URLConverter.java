 ...
 * /reflectutils/converters/URLConverter.java $
	...
	public URL convert(final Object value) {
		...
		if ((s != null) && (s.length() > 0)) {
			try {
				...
				throw new UnsupportedOperationException("URL convert failure: cannot convert source (" + value + ") and type ("
																								+ value.getClass() + ") to a URL: " + e.getMessage());
			}
		}
		if (url == null) {
			throw new UnsupportedOperationException("URL convert failure: cannot convert source (" + value + ") and type ("
																							+ value.getClass() + ") to a URL");
		}
		return url;
	}
