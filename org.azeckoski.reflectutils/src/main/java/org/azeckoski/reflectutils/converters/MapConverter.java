 ...
 * /reflectutils/converters/MapConverter.java $
...
public class MapConverter implements InterfaceConverter<Map> {
	...
	@Override
	public Map convert(final Object value) {
		return this.convertInterface(value, ArrayOrderedMap.class);
	}
	
	@Override
	public Map convertInterface(final Object value, Class<? extends Map> implementationType) {
		Map convert = null;
		...
		if (ConstructorUtils.isClassArray(fromType)) {
			...
			// from map - this is a case where we are going from one map type to
			// another
