 ...
 * /reflectutils/converters/NumberConverter.java $
 ...
 * Special handling for {@link Boolean}, {@link Date}, {@link Calendar}, and
 * {@link String}
 * <ul>
...
public class NumberConverter implements Converter<Number> {
	...
	public static <T> T convertToType(final Class<T> targetType, final Object value) {
		...
		if (value instanceof Calendar) {
			final long time = ((Calendar) value).getTime().getTime();
			return (T) NumberConverter.toNumber(Long.class, targetType, time);
		}
		...
		if (stringValue.length() == 0) {
			...
			if (defaultValue == null) {
				throw new UnsupportedOperationException("Number convert failure: Could not convert empty string to target ("
		...
		final Number number = NumberConverter.toNumber(sourceType, targetType, stringValue);
		return (T) number;
	}
	 ...
	 * @return The converted value.
	 */
	private static Number toNumber(final Class<?> sourceType, final Class<?> targetType, final Number value) {
		...
		if (targetType.equals(Byte.class)) {
			...
			if (longValue > Byte.MAX_VALUE) {
				throw new UnsupportedOperationException("source (" + sourceType + ") value (" + value
																								+ ") is too large for target (" + targetType + ")");
			}
			if (longValue < Byte.MIN_VALUE) {
				throw new UnsupportedOperationException("source (" + sourceType + ") value (" + value
																								+ ") is too small for target (" + targetType + ")");
			}
		...
		throw new UnsupportedOperationException("Number convert failure: cannot convert source (" + sourceType
	...
	private static Number toNumber(final Class<?> sourceType, final Class<?> targetType, final String value) {
		...
		throw new UnsupportedOperationException("Number convert failure: cannot convert string source (" + sourceType
																						+ ") to target (" + targetType + ")");
	}
	
	@Override
	public Number convert(final Object value) {
