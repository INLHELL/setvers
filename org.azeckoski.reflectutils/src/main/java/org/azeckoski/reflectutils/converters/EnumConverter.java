 ...
 * /reflectutils/converters/EnumConverter.java $
...
/**
 * Handles conversions to enums (this is fairly limited to simply converting
 * strings to enums)
 * 
...
public class EnumConverter implements InterfaceConverter<Enum> {
	...
	@Override
	public Enum convertInterface(final Object value, final Class<? extends Enum> implementationType) {
		Enum convert = null;
		final String name = value.toString(); // can only deal with strings
		// now we try to find the enum field in this enum class
		final ClassFields<Enum> cf = (ClassFields<Enum>) this.getFieldUtils().analyzeClass(implementationType);
		final List<Enum> enumConstants = cf.getClassData().getEnumConstants();
		...
		if (convert == null) {
			throw new UnsupportedOperationException("Failure attempting to create enum for name (" + name + ") in ("
																							+ implementationType + ")");
		}
		return convert;
	}
