 ...
 * /reflectutils/converters/api/VariableConverter.java $
...
/**
 * Allows a converter to check to see if it can handle converting something
 * before
 * any of the converters are actually called, these converters are called before
 * any of the other converters if they happen to return true, note that the
 * order
...
public interface VariableConverter extends BaseConverter {
	...
	/**
	 * Convert the specified input object into an output object of the type
	 * specified
	 * 
	 * @param value the input value to be converted, this will never be null or
	 *          the type which you are converting to
